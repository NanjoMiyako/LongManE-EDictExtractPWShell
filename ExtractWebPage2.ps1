...
Write-Output '' | Set-Content -Encoding UTF8 C:\hogehoge\powerShellScript\ExtractURLData1\URLList2.txt

foreach($fName in $fNameList){

    #ファイル名をフルパスに変更
    $fullPathName = 'C:\hogehoge\powerShellScript\htmlPages2\'+$fName
    ...
    foreach ($l in $f1) {
        if($l.indexOf($searchStr1) -ne -1) {
            if($l.indexOf($exceptStr1) -eq -1){
                ...
                Write-Output $l2 | Add-Content -Encoding UTF8 C:\hogehoge\powerShellScript\ExtractURLData1\URLList2.txt
